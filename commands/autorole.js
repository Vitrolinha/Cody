...
module.exports = class Autorole extends command {
    ...
    async run ({message, args, servidor, usuario, prefix}, t) {
        ...
        let role = message.mentions.roles.first() ? message.mentions.roles.first() : message.guild.roles.get(args[1]) ? message.guild.roles.get(args[1]) : message.guild.roles.find(re => re.name === args.splice(1).join(' ')) ? message.guild.roles.find(re => re.name === args.splice(1).join(' ')) : false
