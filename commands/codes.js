...
module.exports = class Decode extends command {
    ...
    async run ({message, usuario}, t) {
        let userDB = message.mentions.users.first() ? this.client.database.Users.findOne({'_id': message.mentions.users.first().id}) ? this.client.database:Users.findOne({'_id': message.mentions.users.first().id}) : usuario : usuario
