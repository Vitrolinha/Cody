        ...
        if(config === 'prefix') {
            ...
            let genEmbed = async(doc) => {
                ...
                let sucess = new this.client.Discord.RichEmbed()
                    ...
                    .addField(t('comandos:config.cmdChannel.allowed'), allowedChannels.length === 0 ? t('comandos:config.cmdChannel.emptyAlloweds') : allowedChannels.map(channel => `**${message.guild.channels.get(channel).name}**`).join(', '))
                    .addField(t('comandos:config.cmdChannel.denied'), deniedChannels.length === 0 ? t('comandos:config.cmdChannel.emptyDenieds') : deniedChannels.map(channel => `**${message.guild.channels.get(channel).name}**`).join(', '))
                    .setTimestamp(new Date())
                    .setFooter(message.author.username, message.author.displayAvatarURL)
                    .setColor(5289)
                return sucess;
