        ...
        if(config === 'prefix') {
            ...
            let genEmbed = async(doc) => {
                ...
                let sucess = new this.client.Discord.RichEmbed()
                    ...
                    .addField(t('comandos:config.cmdChannel.allowed'), allowedChannels.length === 0 ? t('comandos:config.cmdChannel.emptyAlloweds') : allowMap.length >= 1000 ? t('comandos:config.cmdChannel.manyChannels') : allowMap)
                    .addField(t('comandos:config.cmdChannel.denied'), deniedChannels.length === 0 ? t('comandos:config.cmdChannel.emptyDenieds') : denyMap.length >= 1000 ? t('comandos:config.cmdChannel.manyChannels') : denyMap)
                    .setTimestamp(new Date())
                    .setFooter(message.author.username, message.author.displayAvatarURL)
                    .setColor(5289)
                return sucess;
