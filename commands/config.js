...
module.exports = class Config extends command {
    ...
    async run ({message, args, prefix, usuario, servidor}, t) {
        ...
        let embed = new this.client.Discord.RichEmbed()
            ...
            .setColor(2631906)
        if (!args[0] && !configs.includes(args[0].toLowerCase())) return message.channel.send(embed);
        ...
        if (config === 'prefix') {
            ...
            let sugest = new this.client.Discord.RichEmbed()
                ...
                .setFooter(message.author.username, message.author.displayAvatarURL)
                .setColor(2631906)
            if(!args[1] && !actions.includes(args[1].toLowerCase())) return message.channel.send(sugest);
